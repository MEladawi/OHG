...
GSEA keeps the ranking and is the right tool for many questions: its enrichment score is a running statistic that weights each gene by the magnitude of the ranking metric (the fgsea exponent, default 1), well suited to broad, coordinated shifts. It asks a different question than OHG — OHG uses the metric only to order genes and then asks, by count, whether a pathway's genes are concentrated in the top prefix, whereas GSEA's score also reflects how large those metric values are.
...
The normalization follows the same idea as GSEA's NES — standardize the observed statistic against its permutation null for gene sets of the same size. GSEA rescales its enrichment score by the null enrichment scores; `NLES` does the analogous thing for magnitude, as the robust z-score above (the observed leading-edge median, minus the null median, divided by the null MAD). The difference is *what* is normalized: GSEA's NES normalizes its **significance** statistic, so significance and magnitude move together, whereas `NLES` normalizes a separate **magnitude** axis and is never folded into the p-value or the FDR correction. They stay two independent readouts, because a pathway can be highly significant on a modest effect or modestly significant on a large one, and a single combined number cannot tell you which — so OHG keeps them apart and lets you combine them yourself when you build a shortlist. `NLES_signed` carries the up/down direction.
